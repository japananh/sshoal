...
- **From the CLI** — drive the running app without opening it: `sshoal list`, `sshoal connect PATH`, `sshoal disconnect PATH`, `sshoal status PATH`, `sshoal set PATH …`. `PATH` is a full tunnel path or a folder (connects everything under it). `connect` waits until each tunnel is up and exits non-zero if any didn't (so scripts and agents can gate on it); `--no-wait` to fire-and-forget. `set` edits one tunnel's fields — the same ones the app's edit screen has — e.g. `sshoal set gc/dev/db/app-api --local-port 54399 --remote-host db.internal` (also `--folder`, `--name`, `--ssh`, `--remote-port`); a connected tunnel reconnects on the new settings. Needs the app running (over a private per-user socket at `~/.config/sshoal/control.sock`).
